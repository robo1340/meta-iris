...
class SSC32:
	def __init__(self, path, baud, timeout, servo_list : list, increment_amount):
		self.path = path
		self.baud = baud
...
if __name__=="__main__":
	...
	parser.add_argument('-s',  '--servos', type=str, default='28,29,30,12,13,14,20,21,22,4,5,6,16,17,18,0,1,2', help='comma delimited list of servo channels')
	parser.add_argument('-i',  '--increment', type=int, default=25, help='')
	argss = parser.parse_args()
	args = argss.__dict__
	
	servos = [int(x) for x in args['servos'].split(',')]
	ssc32 = SSC32('/dev/ttySTM1', 115200, 2.0, servos, args['increment'])
