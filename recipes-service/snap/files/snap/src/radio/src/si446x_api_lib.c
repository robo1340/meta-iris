...
uint8_t si446x_configuration_init(const uint8_t * pSetPropCmd){
	printf("INFO: si446x_configuration_init()\n");
	uint8_t col;
	uint8_t numOfBytes;
...
U8 si446x_apply_patch(void) {
	printf("INFO: si446x_apply_patch()\n");
	uint16_t line, row;
	...
	if ((Si446xCmd.PART_INFO.ROMID == SI446X_PATCH_ROMID) && (msb < SI446X_PATCH_ID)) {
		...
		for (line=0; line<(sizeof(Si446xPatchCommands)/8u); line++){
			...
			if (radio_comm_SendCmdGetResp(8, Pro2Cmd, 0, 0) != 0xFF){// Timeout occured
				return SI446X_CTS_TIMEOUT;
			}
			if (radio_hal_NirqLevel() == 0){// Get and clear all interrupts.  An error has occured...
				si446x_get_int_status(0, 0, 0);
...
void si446x_get_int_status(U8 PH_CLR_PEND, U8 MODEM_CLR_PEND, U8 CHIP_CLR_PEND)
{
	...
	//printf("%x %x %x %x\n", Pro2Cmd[0], Pro2Cmd[1], Pro2Cmd[2], Pro2Cmd[3]);
