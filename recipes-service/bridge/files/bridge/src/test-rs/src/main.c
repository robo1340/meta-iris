...
uint32_t main()
{
    ...
    { // decode_message test (WITH 3 ERRORS)
        uint32_t t = 2;
        uint32_t C[] = {69, 211, 228, 3, 1, 2};
        uint8_t expected[] = "hi";
        uint8_t *codeword = vector_to_str(C, LEN(C));
        uint32_t out_len = 0;
        uint8_t *M = decode_message(codeword, LEN(C), t, &out_len);
	...
	{ // encode_message test
        uint8_t msg[] = "this is a long message that goes on for a bit and says many things that keep on going on and on and on";
        uint32_t actual_len = 0;
        uint8_t *actual = encode_message(msg, sizeof(msg), 2, &actual_len);
        actual[1] = 0;
		actual[2] = 0;
		uint32_t out_len;
        uint8_t *M = decode_message(actual, actual_len, 2, &out_len);
		assert(memcmp(msg, M, out_len)==0);
        free(M);
		free(actual);
    }
