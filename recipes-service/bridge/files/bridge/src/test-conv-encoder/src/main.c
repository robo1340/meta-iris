...
static int error_test(uint8_t type){
	...
	conv_encoder_t * enc = conv_encoder(type);
	...
	uint8_t * coded = conv_encoder_encode(enc, input, enc->frame_bytes_len);
	assert(coded != NULL);
	...
	*/
	...
	uint8_t * decoded = conv_encoder_decode(enc, coded, enc->coded_bytes_len*enc->conv_blocks);
	if (decoded == NULL){
