...
#define HDR_UNCODED_BYTES 24 //supports WiMax_FCH
#define HDR_CONV_RS_DATA	20 //reed solomon data symbols per block
#define HDR_CONV_RS_ECC 4 //reed solomon error correction symbols per block
//HDR_CONV_RS_DATA+HDR_CONV_RS_ECC must be a divisor of HDR_UNCODED_BYTES
...
conv_encoder_t * hdr_conv_encoder(void){	
	...
	to_return->enc = encoder(WiMax_FCH);
	...
	//if (type == WiMax_FCH){
	to_return->conv_blocks = HDR_UNCODED_BYTES/to_return->uncoded_bytes_len;
	//}
	
	to_return->rs_block_len = HDR_CONV_RS_DATA+HDR_CONV_RS_ECC;
	...
	if ((to_return->uncoded_bytes_len*to_return->conv_blocks % to_return->rs_block_len) != 0){
		printf("ERROR turbo_encoder HDR_CONV_RS_DATA+HDR_CONV_RS_ECC does not evenly divide uncoded bytes length\n");
		printf("%u %u %u\n", to_return->uncoded_bytes_len, to_return->conv_blocks, to_return->rs_block_len);
		return NULL;
	}
	to_return->frame_bytes_len = (to_return->uncoded_bytes_len*to_return->conv_blocks) - HDR_CONV_RS_ECC*to_return->rs_blocks;
...
uint8_t * hdr_conv_encoder_encode(conv_encoder_t * encoder, uint8_t * input, uint32_t input_len){
	//printf("INFO: hdr_conv_encoder_encode()\n");
	uint32_t i,j;
	//int rc;
	uint32_t actual_len = 0;
	uint8_t * rs_block = NULL;
	...
	//Reed solomon encoding
	for (i=0,j=0; i<input_len; i+=HDR_CONV_RS_DATA,j+=encoder->rs_block_len){
		//printArrHex(&input[i], HDR_CONV_RS_DATA);
		rs_block = encode_message(&input[i], HDR_CONV_RS_DATA, 2, &actual_len); //rs encode
		memcpy(&encoder->uncoded_bytes[j], rs_block, encoder->rs_block_len);
		free(rs_block);
...
uint8_t * hdr_conv_encoder_decode(conv_encoder_t * encoder, uint8_t * input, uint32_t input_len){
	//printf("INFO: hdr_conv_encoder_decode()\n");
	uint32_t i,j;
	int rc;
	uint32_t len;
	uint8_t * rs_block;
	...
	if (input_len != encoder->coded_bytes_len*encoder->conv_blocks){
		printf("WARNING: hdr_conv_encoder_decode(%u) expected input length of %u\n", encoder->enc->type, encoder->coded_bytes_len*encoder->conv_blocks);
		return NULL;
	}
	...
	*/
	...
	for (i=0,j=0,rc=0; j<encoder->uncoded_bytes_len*encoder->conv_blocks; i+=HDR_CONV_RS_DATA,j+=encoder->rs_block_len,rc++){
		//printf("%d,%d,%u\n",i,j,encoder->rs_block_len);
		rs_block = decode_message(&encoder->uncoded_bytes[j], encoder->rs_block_len, 2, &len);
		if (rs_block == NULL){
			printf("WARNING: reed solomon decoding failed on block %d\n", rc);
			return NULL;
		}
		assert(len==HDR_CONV_RS_DATA); 
		memcpy(&encoder->coded_bytes[i], rs_block, HDR_CONV_RS_DATA);
		free(rs_block);
	}
...
#ifdef OBJECT_DESTROY_DEBUG
    printf("destroying hdr_conv_encoder_t at 0x%p\n", (void*)*to_destroy);
#endif
...
conv_encoder_t * conv_encoder(uint8_t type){	
	...
	//printf("conv_encoder frame len = %u\n",to_return->frame_bytes_len);
