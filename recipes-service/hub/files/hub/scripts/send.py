...
if __name__ == "__main__":
	...
	parser.add_argument('-e',   '--endpoint', type=str, default="ipc:///tmp/transmit_msg.ipc", help='the tcp socket used for request/response communication')
	args = parser.parse_args()
	config = args.__dict__
	
	if (config['payload_path'] == ''):
		if (config['payload'] == '') and (not sys.stdin.isatty()):
			config['payload'] = json.loads(open(0).read())
	else: #load payload from file
		config['payload'] = json.loads(safe_read_file(config['payload_path']))
	...
	try:
		...
		socket.send_json(config['payload'])
	except BaseException as ex:
		print(ex)
	finally:
		socket.close()
