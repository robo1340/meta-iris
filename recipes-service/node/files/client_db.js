...
class User {
	...
	static add(addr, callsign=null, lat=null, lon=null, type=null, complete_cb=do_nothing){
		...
		const addRequest = objectStore.add(new_user);	
		addRequest.onsuccess = function(){
			complete_cb(new_user);
		};
	}
	...
	static update(addr, callsign=null, lat=null, lon=null, timestamp=null, type=null, complete_cb=do_nothing){ 
		...
		read_request.onsuccess = function() {
			if (read_request.result === undefined){
				if ((lat !== null) && (lon !== null)){
					User.add(addr, callsign, lat, lon, type, complete_cb=complete_cb);
				} else {
					User.add(addr, callsign, null, null, type, complete_cb=complete_cb);
				}
			} else { //update user
